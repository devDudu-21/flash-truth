...
@test "Test English language output" {
  run env LANGUAGE=en /home/devdudu/workplace/flash-truth/flash-truth.sh --mock <<< "0"
  [ "$status" -eq 0 ]
...
@test "Test Portuguese language output" {
  run env LANGUAGE=pt /home/devdudu/workplace/flash-truth/flash-truth.sh --mock <<< "0"
  [ "$status" -eq 0 ]
